...
.factory('$mdcMedia', mdMediaFactory);
 ...
 *
 * @description
 * `$mdcMedia` is used to evaluate whether a given media query is true or false given the
 * current device's screen / window size. The media query will be re-evaluated on resize, allowing
 * you to register a watch.
 *
 * `$mdcMedia` also has pre-programmed support for media queries that match the layout breakpoints:
 *
 ...
 * @usage
 * <hljs lang="js">
 * app.controller('MyController', function($mdcMedia, $scope) {
 *   $scope.$watch(function() { return $mdcMedia('lg'); }, function(big) {
 *     $scope.bigScreen = big;
 *   });
 *
 *   $scope.screenIsSmall = $mdcMedia('sm');
 *   $scope.customQuery = $mdcMedia('(min-width: 1234px)');
 *   $scope.anotherCustom = $mdcMedia('max-width: 300px');
 * });
 * </hljs>
 */
...
function mdMediaFactory($mdConstant, $rootScope, $window) {
  ...
  $mdcMedia.getResponsiveAttribute = getResponsiveAttribute;
  $mdcMedia.getQuery = getQuery;
  $mdcMedia.watchResponsiveAttributes = watchResponsiveAttributes;

  return $mdcMedia;

  function $mdcMedia(query) {
